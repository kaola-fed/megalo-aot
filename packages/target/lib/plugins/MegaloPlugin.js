...
function lazyEmit( compiler, megaloTemplateCompiler, megaloOptions ) {
  ...
  compiler.hooks.emit.tap(
    ...
    compilation => {
      ...
      //  sort when entries start with ^ in pages field
      if (appConfig.pages) {
        appConfig.pages = appConfig.pages.sort((p1, p2) => {
          return p2.startsWith('^') - p1.startsWith('^')
        })
        appConfig.pages = appConfig.pages.map(p => {
          return p.replace(/^\^+/, '')
        })
      }
